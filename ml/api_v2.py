...
async def lifespan(app: FastAPI):
    ...
    # Get current script directory to resolve paths robustly
    base_dir = os.path.dirname(os.path.abspath(__file__))
    
    # 1. Load Tabular Decision Model
    model_path = os.path.join(base_dir, 'bingo_model.keras')
    ...
    if os.path.exists(model_path):
        try:
            ...
            print(f"[Lifespan] Tabular Model loaded successfully from: {model_path}")
        except Exception as e:
            print(f"[Lifespan] ERROR: Failed to load Tabular Model: {e}")
    else:
        print(f"[Lifespan] ERROR: Tabular Model not found at {model_path}")
        
    if os.path.exists(encoders_path):
        try:
            with open(encoders_path, 'rb') as f:
                encoders = pickle.load(f)
            print(f"[Lifespan] Label Encoders loaded successfully from: {encoders_path}")
        except Exception as e:
            print(f"[Lifespan] ERROR: Failed to load encoders: {e}")
    else:
        print(f"[Lifespan] ERROR: Encoders not found at {encoders_path}")
        
    # 2. Load CV Model
    cv_paths = [
        os.path.join(base_dir, 'waste_classifier.keras'),
        os.path.join(base_dir, 'waste_classifier_final.h5'),
        os.path.join(base_dir, '..', 'cv', 'waste_classifier_final.h5'),
        os.path.join(base_dir, '..', 'cv', 'model', 'waste_classifier.h5')
    ]
    ...
    for path in cv_paths:
        if os.path.exists(path):
            try:
                ...
                print(f"[Lifespan] CV Model successfully loaded from: {path}")
                break
            except Exception as e:
                print(f"[Lifespan] Warning: Failed to load CV model from {path}. Error: {e}")
    
    if cv_model is None:
        print("[Lifespan] ERROR: CV Model could not be loaded from any expected path.")
...
# ==========================================
# 3. SCHEMAS FOR API INPUTS & OUTPUTS
# ==========================================
...
async def predict(request: PredictionRequest):
    ...
    try:
        ...
        if request.generate_tips:
            prompt = (
                f"Benda terdeteksi sebagai {request.category}. Status bisa didaur ulang: {recyclable}. "
                f"Metode treatment pabrik: {treatment}. Berikan maksimal 2 kalimat tips singkat dan ramah "
                f"untuk pengguna awam tentang bagaimana sebaiknya menangani sampah ini di rumah sebelum dibuang."
            )
            try:
...
async def predict_hybrid(
    ...
    try:
        ...
        if generate_tips:
            prompt = (
                f"Benda terdeteksi sebagai {cv_category}. Status bisa didaur ulang: {recyclable}. "
                f"Metode treatment pabrik: {treatment}. Berikan maksimal 2 kalimat tips singkat dan ramah "
                f"untuk pengguna awam tentang bagaimana sebaiknya menangani sampah ini di rumah sebelum dibuang."
            )
            try:
...
def get_questionnaire(category: str):
    ...
    if category_lower == "plastic" or category_lower == "kristal":
        specific_questions = [
...
if __name__ == "__main__":
    ...
    uvicorn.run("api_v2:app", host="127.0.0.1", port=7860, reload=True)
