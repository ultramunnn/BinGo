...
export async function sendEmailVerificationEmail(
  ...
  try {
    const templatePath = path.join(process.cwd(), "src/templates/emails/verify-email.html");
    const template = fs.readFileSync(templatePath, "utf8");
    const html = template.replace(/\{\{VERIFY_LINK\}\}/g, verifyLink);

    // If BREVO_API_KEY is present, use Brevo's HTTPS REST API (bypasses Railway SMTP port blocking)
    if (process.env.BREVO_API_KEY) {
      console.log("[Email] Mengirim email verifikasi menggunakan Brevo API...");
      const response = await fetch("https://api.brevo.com/v3/smtp/email", {
        method: "POST",
        headers: {
          "accept": "application/json",
        ...
        body: JSON.stringify({
          ...
          to: [{ email: toEmail }],
          subject: "Verifikasi Email Anda - BinGo",
          htmlContent: html,
        }),
      });
      ...
      console.log("[Email] Verifikasi terkirim via Brevo ke", toEmail, "- MessageID:", resData.messageId);
      return;
    }

    // If RESEND_API_KEY is present, use Resend's HTTPS REST API (bypasses Railway SMTP port blocking)
    if (process.env.RESEND_API_KEY) {
      console.log("[Email] Mengirim email verifikasi menggunakan Resend API...");
      const response = await fetch("https://api.resend.com/emails", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      console.log("[Email] Verifikasi terkirim via Resend ke", toEmail, "- ID:", resData.id);
      return;
    }

    // Fallback to Nodemailer SMTP (e.g. for local development or if SMTP ports are open)
    console.log("[Email] Mengirim email verifikasi menggunakan SMTP Nodemailer...");
    const info = await transporter.sendMail({
      from: `"BinGo" <${process.env.SMTP_EMAIL || "noreply@bingo.app"}>`,
      to: toEmail,
      subject: "Verifikasi Email Anda - BinGo",
    ...
    console.log("[Email] Verifikasi terkirim via SMTP ke", toEmail, "- MessageID:", info.messageId);
  } catch (error: any) {
    console.error("[Email] Gagal kirim verifikasi:", error.message);
    throw error;
  }
}
